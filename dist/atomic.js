...
class AtomicFileManager {
    ...
    ensureDirectories() {
        ...
        for (const dir of dirs) {
            const dirPath = path_1.default.join(this.atomicDir, dir);
     ...
     */
    writeAtomicFact(fact, metadata) {
        const subdir = this.getSubdirectory(fact.type);
        const filename = this.generateFilename(fact);
        const filepath = path_1.default.join(this.atomicDir, subdir, filename);
     ...
     */
    updateAtomicFact(fact, metadata) {
     ...
     */
    addRelation(fromFactId, toFactPath, description) {
     ...
     */
    getAtomicFilesForSystem(system) {
        const files = [];
    ...
    factToAtomicMarkdown(fact, metadata) {
