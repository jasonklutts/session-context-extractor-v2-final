...
const workspacePath = process.env.OPENCLAW_WORKSPACE || path_1.default.join(process.env.HOME || '', '.openclaw', 'workspace');
const command = process.argv[2];
const args = process.argv.slice(3);
async function main() {
    try {
        // Ensure vault directory exists
        const vaultDir = path_1.default.join(workspacePath, 'context-vault');
        if (!fs_1.default.existsSync(vaultDir)) {
            fs_1.default.mkdirSync(vaultDir, { recursive: true });
        }
        const db = new db_1.VaultDatabase(workspacePath);
        await db.initialize();
        const writer = new vault_writer_1.VaultWriter(workspacePath);
        const engine = new query_engine_1.QueryEngine(db);
        switch (command) {
            case 'query': {
                ...
                for (const result of results.slice(0, 5)) {
                    ...
                    if (fact.system)
                        console.log(`  System: ${fact.system}`);
                    console.log();
                }
                break;
            }
            ...
            case 'export': {
                ...
                if (format === 'json') {
                    ...
                    for (const fact of recent) {
                        console.log(`"${fact.id}","${fact.type}","${fact.title}","${fact.timestamp}",${fact.verified ? 'yes' : 'no'},"${fact.system || ''}"`);
                    }
                }
                break;
            }
